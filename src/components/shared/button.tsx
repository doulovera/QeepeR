...
export const Button = ({
  ...
  return (
    <button
      className={`flex cursor-pointer items-center border-base border-base-400 rounded-base ${colors[color].background} ${colors[color].disabled} ${sizes[size]} font-base shadow-dark transition-all hover:translate-x-boxShadowX hover:translate-y-boxShadowY hover:shadow-none disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-x-0 disabled:hover:translate-y-0 disabled:hover:shadow-dark ${className}`}
      title={title}
      disabled={disabled}
      type={type}
      onClick={onClick}
